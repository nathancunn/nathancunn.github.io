...
top_ten <- data.frame(name = c("Mr Burns", "Moe Szyslak", "Ned Flanders", "Seymour Skinner", "Krusty the Klown",
                               "Milhouse", "Chief Wiggum", "Lenny", "Grampa", "Carl", 
                               "Selma", "Dr Hibbert", "Barney", "Nelson", "Waylon Smithers", 
                               "Groundskeeper Willie", "Patty", "Mayor Quimby", "Apu", "Professor Frink"),
                      count = c(160, 138, 111, 106, 103, 107, 97, 82, 86, 75, 71, 66, 60,
                                61, 57, 54, 53, 53, 47, 47))

old_season <- data.frame(name = c("Mr Burns", "Krusty", "Moe", "Ned Flanders", "Grampa",
                                  "Seymour Skinner", "Barney", "Milhouse", "Chief Wiggum", "Troy McClure",
                                  "Waylon Smithers", "Itchy", "Scratchy", "Lionel Hutz", "Selma",
                                  "Kent Brockman", "Carl", "Patty", "Apu", "Groundskeeper Willie"),
                      count = c(78, 53, 51, 48, 45, 41, 39, 37, 35, 35, 34, 30, 29, 26, 25, 24, 24, 24, 23, 23))
library(ggplot2)
```
...
ggplot(top_ten, aes(x = reorder(name, count), y = count)) + 
  geom_col(fill = "#FFD43A") + 
  coord_flip() +
  ggtitle("     Who is the main side character in The Simpsons?") + 
  theme_minimal() +
  labs(caption = "Source: nathancunn.com (@nathcun); data from Wikipedia") +
  ylab("# of episodes featured in") +
  theme(plot.background = element_rect(fill = "#7099E1", colour = "#7099E1"),
        plot.title = element_text(hjust = 0, size = rel(1.1), face = "bold"),
        text = element_text(colour = "#FFFFFFCC", family = "sans"),
        axis.text = element_text(colour = "#FFFFFFCC", size = 10),
...
```{r, out.width = '85%', fig.align = 'center', echo = FALSE}
ggplot(old_season, aes(x = reorder(name, count), y = count)) + 
  geom_col(fill = "#FFD43A") + 
  coord_flip() +
  ggtitle("     Who is the main side character in The Simpsons? (seasons 1-9)") + 
  theme_minimal() +
  labs(caption = "Source: nathancunn.com (@nathcun); data from Wikipedia") +
  ylab("# of episodes featured in") +
  theme(plot.background = element_rect(fill = "#7099E1", colour = "#7099E1"),
